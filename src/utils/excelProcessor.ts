...
export function getRowValue(row: any, searchKeys: string[]): string {
  ...
  for (const key of Object.keys(row)) {
    ...
    for (const searchKey of searchKeys) {
      const cleanSearchKey = searchKey
        .toLowerCase()
        .replace(/[\(\)\s_-]/g, "")
        .trim();
...
export function categorizeInputStatus(
  val: string | undefined,
): "Mất mạng" | "Wifi yếu" | "Camera lỗi" | "Chập chờn" | "Khác" {
  if (!val) return "Khác";
...
export function processCLLData(
  ...
  for (let i = 0; i < cllRows.length; i++) {
    ...
    const inputStatusL2 = getRowValue(row, [
      "Tình trạng đầu vào",
      "tình trạng đầu vào",
      "status",
    ]);
    const errorElementL2 = getRowValue(row, [
      "Phần tử lỗi",
      "phần tử lỗi",
      "element",
    ]);
    const errorCauseL2 = getRowValue(row, [
      "Nguyên nhân lỗi",
      "nguyên nhân lỗi",
      "cause",
    ]);
    const handlingL2 = getRowValue(row, [
      "Hướng xử lý",
      "hướng xử lý",
      "handling",
    ]);
    const staffL2 = getRowValue(row, [
      "Nhân viên kỹ thuật",
      "Kỹ thuật",
      "Kỹ thuật thực hiện",
      "Nhân viên thực hiện",
    ...
    let blockL2 = getRowValue(row, ["Block", "Khu vực", "Khu vuc"]);
        ...
        for (const cand of candidates) {
          ...
          if (candTimeDate) {
            const diffMs = Math.abs(
              candTimeDate.getTime() - l2TimeDate.getTime(),
            );
    ...
    if (bestMatch) {
      ...
      const inputStatusL1 = getRowValue(bestMatch, [
        "Tình trạng đầu vào",
        "tình trạng đầu vào",
        "status",
      ]);
      const errorElementL1 = getRowValue(bestMatch, [
        "Phần tử lỗi",
        "phần tử lỗi",
        "element",
      ]);
      const errorCauseL1 = getRowValue(bestMatch, [
        "Nguyên nhân lỗi",
        "nguyên nhân lỗi",
        "cause",
      ]);
      const handlingL1 = getRowValue(bestMatch, [
        "Hướng xử lý",
        "hướng xử lý",
        "handling",
      ]);
      const staffL1 = getRowValue(bestMatch, [
        "Nhân viên kỹ thuật",
        "Kỹ thuật",
        "Kỹ thuật thực hiện",
        "Nhân viên thực hiện",
      ...
      const blockL1 = getRowValue(bestMatch, ["Block", "Khu vực", "Khu vuc"]);
      blockL2 = blockL1; // Both L1 and L2 take from "đúng hẹn tháng T"
      ...
      mergedRecords.push({
        ...
        timeDifferenceMinutes:
          minDiffMinutes !== Infinity ? Math.round(minDiffMinutes) : undefined,
        staffL1,
        staffL2,
        blockL1,
        blockL2,
...
export function calculateStatistics(records: MergedRecord[]) {
  ...
  const blockL1Counts: Record<string, number> = {};
  const blockL2Counts: Record<string, number> = {};
  ...
  for (const rec of records) {
    ...
    // Khu vực (Block)
    const bkL1 = rec.blockL1 ? rec.blockL1.trim() : "(Trống)";
    blockL1Counts[bkL1] = (blockL1Counts[bkL1] || 0) + 1;

    const bkL2 = rec.blockL2 ? rec.blockL2.trim() : "(Trống)";
    blockL2Counts[bkL2] = (blockL2Counts[bkL2] || 0) + 1;
  ...
  const getSortedChartData = (
    countsL1: Record<string, number>,
    countsL2: Record<string, number>,
  ): ChartDataItem[] => {
    const allKeys = Array.from(
      new Set([...Object.keys(countsL1), ...Object.keys(countsL2)]),
    );
    return allKeys
      .map((key) => ({
        name: key,
  ...
  const chartInputStatus = getSortedChartData(
    inputStatusCountsL1,
    inputStatusCountsL2,
  );
  const chartErrorElement = getSortedChartData(
    errorElementCountsL1,
    errorElementCountsL2,
  );
  const chartErrorCause = getSortedChartData(
    errorCauseCountsL1,
    errorCauseCountsL2,
  );
  ...
  const chartBlock = getSortedChartData(blockL1Counts, blockL2Counts);
...
export async function exportToExcel(
  ...
  for (const record of records) {
    ...
    row.getCell(9).value =
      record.status === "SUCCESS" ? "Ghép thành công" : "Không tìm thấy CLPS";
    ...
    row.getCell(17).value =
      record.timeDifferenceMinutes !== undefined
        ? record.timeDifferenceMinutes
        : "";
  ...
  tsHeader.height = 24;
  const tsHeaders = [
    "Nguyên nhân lỗi Lần 1",
    "Nguyên nhân lỗi Lần 2",
  ...
  stats.transitions.forEach((trans: TransitionRecord, rIdx: number) => {
    ...
    row.getCell(4).value =
      totalTransitions > 0
        ? ((trans.count / totalTransitions) * 100).toFixed(2) + "%"
        : "0%";
