...
const Top10StaffTable = ({
  ...
  if (data.length === 0)
    return (
      <div className="text-[10px] italic text-slate-500 font-mono">
        Không có dữ liệu
      </div>
    );
  ...
  const renderRow = ([name, count]: [string, number], index: number) => (
    <tr
      key={name}
      className={`border-b ${theme === "slate" ? "border-slate-200 hover:bg-slate-50" : "border-emerald-100 hover:bg-emerald-50"} last:border-0`}
    >
      <td
        className={`p-1.5 text-center font-black ${theme === "slate" ? "text-slate-500 border-r border-slate-200" : "text-emerald-600 border-r border-emerald-100"}`}
      >
        {index + 1}
      </td>
      <td
        className={`p-1.5 font-bold truncate max-w-[150px] sm:max-w-[180px] ${theme === "slate" ? "text-slate-800 border-r border-slate-200" : "text-emerald-900 border-r border-emerald-100"}`}
        title={name}
      >
        {name || "(Trống)"}
      </td>
      <td
        className={`p-1.5 text-right font-black ${theme === "slate" ? "text-slate-900" : "text-emerald-700"}`}
      >
        {count}
      </td>
    </tr>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div
        className={`overflow-hidden border ${theme === "slate" ? "border-slate-900" : "border-emerald-800"}`}
      >
        <table className="w-full text-left border-collapse bg-white">
          <thead>
            <tr
              className={`text-[9px] uppercase tracking-widest font-black font-mono ${theme === "slate" ? "bg-slate-900 text-white" : "bg-emerald-800 text-white"}`}
            >
              <th className="p-1.5 w-10 text-center border-r border-white/20">
                STT
              </th>
...
export default function CombinedPreviewTable({
  records,
  chartFilter,
  onClearChartFilter,
}: CombinedPreviewTableProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "ALL" | "SUCCESS" | "NOT_FOUND"
  >("ALL");
  const [isExporting, setIsExporting] = useState(false);

  // L1 State Variables for Filters
  const [selectedInputStatusesL1, setSelectedInputStatusesL1] = useState<
    string[]
  >([]);
  const [selectedErrorElementsL1, setSelectedErrorElementsL1] = useState<
    string[]
  >([]);
  ...
  const [selectedBlocksL1, setSelectedBlocksL1] = useState<string[]>([]);
  ...
  const [blockL1Search, setBlockL1Search] = useState("");

  // L2 State Variables for Filters
  const [selectedInputStatusesL2, setSelectedInputStatusesL2] = useState<
    string[]
  >([]);
  const [selectedErrorElementsL2, setSelectedErrorElementsL2] = useState<
    string[]
  >([]);
  ...
  const [selectedBlocksL2, setSelectedBlocksL2] = useState<string[]>([]);
  ...
  const [blockL2Search, setBlockL2Search] = useState("");
  ...
  const uniqueBlocksL1 = React.useMemo(() => {
    const values = new Set<string>();
    records.forEach((r) => {
      if (r.blockL1) values.add(r.blockL1);
    });
    return Array.from(values).sort();
  }, [records]);
  ...
  const uniqueBlocksL2 = React.useMemo(() => {
    const values = new Set<string>();
    records.forEach((r) => {
      if (r.blockL2) values.add(r.blockL2);
    });
    return Array.from(values).sort();
  }, [records]);
  ...
  const valueCounts = React.useMemo(() => {
    const counts = {
      ...
      blockL1: {} as Record<string, number>,
      ...
      blockL2: {} as Record<string, number>,
    };

    records.forEach((r) => {
      if (r.inputStatusL1)
        counts.inputStatusL1[r.inputStatusL1] =
          (counts.inputStatusL1[r.inputStatusL1] || 0) + 1;
      if (r.errorElementL1)
        counts.errorElementL1[r.errorElementL1] =
          (counts.errorElementL1[r.errorElementL1] || 0) + 1;
      if (r.errorCauseL1)
        counts.errorCauseL1[r.errorCauseL1] =
          (counts.errorCauseL1[r.errorCauseL1] || 0) + 1;
      if (r.handlingL1)
        counts.handlingL1[r.handlingL1] =
          (counts.handlingL1[r.handlingL1] || 0) + 1;
      if (r.staffL1)
        counts.staffL1[r.staffL1] = (counts.staffL1[r.staffL1] || 0) + 1;
      if (r.blockL1)
        counts.blockL1[r.blockL1] = (counts.blockL1[r.blockL1] || 0) + 1;

      if (r.inputStatusL2)
        counts.inputStatusL2[r.inputStatusL2] =
          (counts.inputStatusL2[r.inputStatusL2] || 0) + 1;
      if (r.errorElementL2)
        counts.errorElementL2[r.errorElementL2] =
          (counts.errorElementL2[r.errorElementL2] || 0) + 1;
      if (r.errorCauseL2)
        counts.errorCauseL2[r.errorCauseL2] =
          (counts.errorCauseL2[r.errorCauseL2] || 0) + 1;
      if (r.handlingL2)
        counts.handlingL2[r.handlingL2] =
          (counts.handlingL2[r.handlingL2] || 0) + 1;
      if (r.staffL2)
        counts.staffL2[r.staffL2] = (counts.staffL2[r.staffL2] || 0) + 1;
      if (r.blockL2)
        counts.blockL2[r.blockL2] = (counts.blockL2[r.blockL2] || 0) + 1;
    });
  ...
  const filteredInputStatusesL1 = React.useMemo(() => {
    return uniqueInputStatusesL1.filter((val) =>
      val.toLowerCase().includes(inputStatusL1Search.toLowerCase()),
    );
  }, [uniqueInputStatusesL1, inputStatusL1Search]);

  const filteredErrorElementsL1 = React.useMemo(() => {
    return uniqueErrorElementsL1.filter((val) =>
      val.toLowerCase().includes(errorElementL1Search.toLowerCase()),
    );
  }, [uniqueErrorElementsL1, errorElementL1Search]);

  const filteredErrorCausesL1 = React.useMemo(() => {
    return uniqueErrorCausesL1.filter((val) =>
      val.toLowerCase().includes(errorCauseL1Search.toLowerCase()),
    );
  }, [uniqueErrorCausesL1, errorCauseL1Search]);

  const filteredHandlingsL1 = React.useMemo(() => {
    return uniqueHandlingsL1.filter((val) =>
      val.toLowerCase().includes(handlingL1Search.toLowerCase()),
    );
  }, [uniqueHandlingsL1, handlingL1Search]);

  const filteredStaffsL1 = React.useMemo(() => {
    return uniqueStaffsL1.filter((val) =>
      val.toLowerCase().includes(staffL1Search.toLowerCase()),
    );
  }, [uniqueStaffsL1, staffL1Search]);

  const filteredBlocksL1 = React.useMemo(() => {
    return uniqueBlocksL1.filter((val) =>
      val.toLowerCase().includes(blockL1Search.toLowerCase()),
    );
  }, [uniqueBlocksL1, blockL1Search]);

  const filteredInputStatusesL2 = React.useMemo(() => {
    return uniqueInputStatusesL2.filter((val) =>
      val.toLowerCase().includes(inputStatusL2Search.toLowerCase()),
    );
  }, [uniqueInputStatusesL2, inputStatusL2Search]);

  const filteredErrorElementsL2 = React.useMemo(() => {
    return uniqueErrorElementsL2.filter((val) =>
      val.toLowerCase().includes(errorElementL2Search.toLowerCase()),
    );
  }, [uniqueErrorElementsL2, errorElementL2Search]);

  const filteredErrorCausesL2 = React.useMemo(() => {
    return uniqueErrorCausesL2.filter((val) =>
      val.toLowerCase().includes(errorCauseL2Search.toLowerCase()),
    );
  }, [uniqueErrorCausesL2, errorCauseL2Search]);

  const filteredHandlingsL2 = React.useMemo(() => {
    return uniqueHandlingsL2.filter((val) =>
      val.toLowerCase().includes(handlingL2Search.toLowerCase()),
    );
  }, [uniqueHandlingsL2, handlingL2Search]);

  const filteredStaffsL2 = React.useMemo(() => {
    return uniqueStaffsL2.filter((val) =>
      val.toLowerCase().includes(staffL2Search.toLowerCase()),
    );
  }, [uniqueStaffsL2, staffL2Search]);

  const filteredBlocksL2 = React.useMemo(() => {
    return uniqueBlocksL2.filter((val) =>
      val.toLowerCase().includes(blockL2Search.toLowerCase()),
    );
  }, [uniqueBlocksL2, blockL2Search]);
  ...
  const filteredRecords = records.filter((rec) => {
    ...
    const matchesInputStatusL1 =
      ...
      (rec.inputStatusL1 !== undefined &&
        selectedInputStatusesL1.includes(rec.inputStatusL1));

    const matchesErrorElementL1 =
      selectedErrorElementsL1.length === 0 ||
      (rec.errorElementL1 !== undefined &&
        selectedErrorElementsL1.includes(rec.errorElementL1));

    const matchesErrorCauseL1 =
      selectedErrorCausesL1.length === 0 ||
      (rec.errorCauseL1 !== undefined &&
        selectedErrorCausesL1.includes(rec.errorCauseL1));

    const matchesHandlingL1 =
      selectedHandlingsL1.length === 0 ||
      (rec.handlingL1 !== undefined &&
        selectedHandlingsL1.includes(rec.handlingL1));

    const matchesStaffL1 =
      selectedStaffsL1.length === 0 ||
      (rec.staffL1 !== undefined && selectedStaffsL1.includes(rec.staffL1));

    const matchesBlockL1 =
      selectedBlocksL1.length === 0 ||
      (rec.blockL1 !== undefined && selectedBlocksL1.includes(rec.blockL1));

    // 4. L2 Dropdowns
    const matchesInputStatusL2 =
      selectedInputStatusesL2.length === 0 ||
      (rec.inputStatusL2 !== undefined &&
        selectedInputStatusesL2.includes(rec.inputStatusL2));

    const matchesErrorElementL2 =
      selectedErrorElementsL2.length === 0 ||
      (rec.errorElementL2 !== undefined &&
        selectedErrorElementsL2.includes(rec.errorElementL2));

    const matchesErrorCauseL2 =
      selectedErrorCausesL2.length === 0 ||
      (rec.errorCauseL2 !== undefined &&
        selectedErrorCausesL2.includes(rec.errorCauseL2));

    const matchesHandlingL2 =
      selectedHandlingsL2.length === 0 ||
      (rec.handlingL2 !== undefined &&
        selectedHandlingsL2.includes(rec.handlingL2));

    const matchesStaffL2 =
      selectedStaffsL2.length === 0 ||
      (rec.staffL2 !== undefined && selectedStaffsL2.includes(rec.staffL2));

    const matchesBlockL2 =
      selectedBlocksL2.length === 0 ||
      (rec.blockL2 !== undefined && selectedBlocksL2.includes(rec.blockL2));
    ...
    if (chartFilter) {
      ...
      if (type === "inputStatus") {
        matchesChartFilter =
          rec.inputStatusL1 === value || rec.inputStatusL2 === value;
      } else if (type === "errorElement") {
        matchesChartFilter =
          rec.errorElementL1 === value || rec.errorElementL2 === value;
      } else if (type === "errorCause") {
        matchesChartFilter =
          rec.errorCauseL1 === value || rec.errorCauseL2 === value;
      } else if (type === "handling") {
        matchesChartFilter =
          rec.handlingL1 === value || rec.handlingL2 === value;
        ...
        matchesChartFilter = rec.blockL1 === value || rec.blockL2 === value;
      }
    }
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Danh sách chi tiết kết quả tích hợp và so khớp từ các tệp Excel đầu
            vào
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="inline-flex bg-slate-100 p-1 rounded-none border border-slate-900 text-[10px] font-bold tracking-wider uppercase shrink-0 font-mono">
            ...
            >
              CHƯA KHỚP CLPS (
              {records.filter((r) => r.status === "NOT_FOUND").length})
            </button>
          </div>
          ...
          >
            ...
            {isExporting
              ? "Đang xuất..."
              : `Xuất Bộ Lọc (${filteredRecords.length})`}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="border border-slate-900 p-4 shadow-[2px_2px_0px_rgba(15,23,42,0.05)] bg-slate-50">
          <div className="flex items-center gap-2 mb-3 border-b border-slate-300 pb-2.5">
            ...
            <span className="text-[11px] font-black text-slate-900 uppercase font-mono tracking-wider">
              BỘ LỌC LẦN 1: BÁO HỎNG (L1)
            </span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 font-mono">
            ...
            <MultiSelectDropdown
              ...
              onChange={(vals) => {
                setSelectedInputStatusesL1(vals);
                setCurrentPage(1);
              }}
            ...
            <MultiSelectDropdown
              ...
              onChange={(vals) => {
                setSelectedErrorElementsL1(vals);
                setCurrentPage(1);
              }}
            ...
            <MultiSelectDropdown
              ...
              searchPlaceholder="🔍 Tìm khu vực L1..."
              options={filteredBlocksL1}
              selectedValues={selectedBlocksL1}
              onChange={(vals) => {
                setSelectedBlocksL1(vals);
                setCurrentPage(1);
              }}
              searchValue={blockL1Search}
              onSearchChange={setBlockL1Search}
              valueCounts={valueCounts.blockL1}
              allLabel={`-- TẤT CẢ KHU VỰC L1 (${filteredBlocksL1.length}) --`}
              colorTheme="slate"
            />
          </div>
          ...
          <div className="mt-5 pt-4 border-t border-slate-300 w-full xl:w-2/3">
            <h4 className="text-[10px] font-black text-slate-700 uppercase tracking-widest mb-3 font-mono">
              Top 10 Nhân Sự Lần 1 (Theo hệ thống lọc)
            </h4>
        ...
        <div className="border border-slate-900 p-4 shadow-[2px_2px_0px_rgba(16,185,129,0.05)] bg-[#f0fdf4]">
          <div className="flex items-center gap-2 mb-3 border-b border-emerald-300 pb-2.5">
            ...
            <span className="text-[11px] font-black text-emerald-800 uppercase font-mono tracking-wider">
              BỘ LỌC LẦN 2: CLPS / NGHIỆM THU (L2)
            </span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 font-mono">
            ...
            <MultiSelectDropdown
              ...
              onChange={(vals) => {
                setSelectedInputStatusesL2(vals);
                setCurrentPage(1);
              }}
            ...
            <MultiSelectDropdown
              ...
              onChange={(vals) => {
                setSelectedErrorElementsL2(vals);
                setCurrentPage(1);
              }}
            ...
            <MultiSelectDropdown
              label="Khu vực L2"
              placeholder="Chọn khu vực..."
              searchPlaceholder="🔍 Tìm khu vực L2..."
              options={filteredBlocksL2}
              selectedValues={selectedBlocksL2}
              onChange={(vals) => {
                setSelectedBlocksL2(vals);
                setCurrentPage(1);
              }}
              searchValue={blockL2Search}
              onSearchChange={setBlockL2Search}
              valueCounts={valueCounts.blockL2}
              allLabel={`-- TẤT CẢ KHU VỰC L2 (${filteredBlocksL2.length}) --`}
              colorTheme="emerald"
            />
          </div>
          ...
          <div className="mt-5 pt-4 border-t border-emerald-300 w-full xl:w-2/3">
            <h4 className="text-[10px] font-black text-emerald-800 uppercase tracking-widest mb-3 font-mono">
              Top 10 Nhân Sự Lần 2 (Theo hệ thống lọc)
            </h4>
        ...
        <table className="w-full text-left border-collapse table-fixed min-w-[1550px]">
          ...
          <thead>
            <tr className="border-b border-slate-900">
              <th
                rowSpan={2}
                className="p-3 bg-slate-100 text-[10px] font-bold text-slate-700 uppercase tracking-widest text-center w-40 sticky left-0 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.1)] border-r border-slate-300 font-mono"
              >
                Số HĐ
              </th>
              <th
                colSpan={8}
                className="py-4 px-3 bg-slate-900 text-[14px] font-black text-white text-center tracking-wider border-r border-slate-800 uppercase shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)]"
              >
                LẦN 1 (Chất lượng phát sóng - Báo hỏng)
              </th>
              <th
                colSpan={8}
                className="py-4 px-3 bg-emerald-800 text-[14px] font-black text-white text-center tracking-wider uppercase shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)]"
              >
            ...
            <tr className="border-b border-slate-900 bg-slate-50 text-[10px] text-slate-500 font-bold font-mono uppercase tracking-wider">
              {/* L1 Headers (CLPS) */}
              <th className="p-3 bg-slate-100 text-slate-700 text-left w-36 border-r border-slate-300 font-bold uppercase tracking-wider">
                Tg hoàn tất (L1)
              </th>
              <th className="p-3 bg-slate-100 text-slate-700 text-left w-64 border-r border-slate-300 font-bold uppercase tracking-wider">
                Ghi chú (L1)
              </th>
              <th className="p-3 bg-slate-100 text-slate-700 text-left w-44 border-r border-slate-300 font-bold uppercase tracking-wider">
                Tình trạng vào (L1)
              </th>
              <th className="p-3 bg-slate-100 text-slate-700 text-left w-44 border-r border-slate-300 font-bold uppercase tracking-wider">
                Phần tử lỗi (L1)
              ...
              <th className="p-3 bg-slate-100 text-slate-700 text-left w-44 border-r border-slate-800 font-bold uppercase tracking-wider">
                Khu vực (L1)
              </th>

              {/* L2 Headers (CLL) */}
              <th className="p-3 bg-emerald-50 text-emerald-900 text-left w-36 border-r border-emerald-200 font-bold uppercase tracking-wider">
                Tg hoàn tất (L2)
              </th>
              <th className="p-3 bg-emerald-50 text-emerald-900 text-left w-64 border-r border-emerald-200 font-bold uppercase tracking-wider">
                Ghi chú (L2)
              </th>
              <th className="p-3 bg-emerald-50 text-emerald-900 text-left w-44 border-r border-emerald-200 font-bold uppercase tracking-wider">
                Tình trạng vào (L2)
              </th>
              <th className="p-3 bg-emerald-50 text-emerald-900 text-left w-44 border-r border-emerald-200 font-bold uppercase tracking-wider">
                Phần tử lỗi (L2)
              ...
              <th className="p-3 bg-emerald-50 text-emerald-900 text-left w-44 font-bold uppercase tracking-wider">
                Khu vực (L2)
              </th>
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-slate-200 text-xs">
            {paginatedRecords.length === 0 ? (
              <tr>
                <td
                  colSpan={15}
                  className="p-16 text-center text-slate-400 font-mono"
                >
                  <div className="flex flex-col items-center gap-2">
                    <FileX className="h-8 w-8 text-slate-300 stroke-1" />
                    <span className="text-xs uppercase font-bold tracking-wider">
                      Không tìm thấy bản ghi nào khớp các bộ lọc này.
                    </span>
                  </div>
                </td>
              </tr>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3 font-semibold text-slate-900 bg-white sticky left-0 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.06)] border-r border-slate-300 text-center">
                      <span className="font-mono bg-slate-100 border border-slate-200 px-2 py-0.5 text-[11px] font-bold text-slate-800">
                        {item.contractNumber}
                      </span>
                    </td>
                    ...
                    {/* L1 Info */}
                    <td
                      className="p-3 text-slate-600 border-r border-slate-200 truncate font-mono text-[11px]"
                      title={item.completedTimeL1}
                    >
                      {item.completedTimeL1 || "-"}
                    </td>
                    <td
                      className="p-3 text-slate-600 border-r border-slate-200 truncate"
                      title={item.notesL1}
                    >
                      {item.notesL1 || (
                    ...
                    <td
                      className="p-3 text-slate-700 border-r border-slate-200 font-medium truncate"
                      title={item.inputStatusL1}
                    >
                      {item.inputStatusL1 || "-"}
                    </td>
                    <td
                      className="p-3 text-slate-700 border-r border-slate-200 truncate"
                      title={item.errorElementL1}
                    >
                      {item.errorElementL1 || "-"}
                    </td>
                    <td
                      className="p-3 text-slate-700 border-r border-slate-200 font-bold truncate"
                      title={item.errorCauseL1}
                    >
                      {item.errorCauseL1 || "-"}
                    </td>
                    <td
                      className="p-3 text-slate-700 border-r border-slate-200 truncate"
                      title={item.handlingL1}
                    >
                      {item.handlingL1 || "-"}
                    </td>
                    <td
                      className="p-3 text-slate-700 border-r border-slate-200 truncate font-medium text-slate-900"
                      title={item.staffL1}
                    >
                      {item.staffL1 || (
                        <span className="text-slate-300 italic">Trống</span>
                    ...
                    <td
                      className="p-3 text-slate-700 border-r border-slate-300 truncate font-medium text-slate-900"
                      title={item.blockL1}
                    >
                      {item.blockL1 || (
                        <span className="text-slate-300 italic">Trống</span>
                    ...
                    {/* L2 Info */}
                    <td
                      className={`p-3 border-r border-slate-200 truncate font-mono text-[11px] ${isMatched ? "text-slate-600 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                    >
                      {item.completedTimeL2 || "-"}
                    </td>
                    <td
                      className={`p-3 border-r border-slate-200 truncate ${isMatched ? "text-slate-600 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                      title={item.notesL2}
                    >
                      {item.notesL2 || (
                        <span className="text-slate-300 italic">Trống</span>
                    ...
                    <td
                      className={`p-3 border-r border-slate-200 truncate font-medium ${isMatched ? "text-slate-950 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                    >
                      {item.inputStatusL2 || "-"}
                    </td>
                    <td
                      className={`p-3 border-r border-slate-200 truncate ${isMatched ? "text-slate-950 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                    >
                      {item.errorElementL2 || "-"}
                    </td>
                    <td
                      className={`p-3 border-r border-slate-200 font-bold truncate ${isMatched ? "text-emerald-800 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                      title={item.errorCauseL2}
                    >
                      {item.errorCauseL2 || "-"}
                    </td>
                    <td
                      className={`p-3 border-r border-slate-200 truncate font-medium ${isMatched ? "text-slate-950 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                      title={item.handlingL2}
                    >
                      {item.handlingL2 || "-"}
                    </td>
                    <td
                      className={`p-3 border-r border-slate-200 truncate font-medium ${isMatched ? "text-slate-900 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                      title={item.staffL2}
                    >
                      {item.staffL2 || (
                        <span className="text-slate-300 italic">Trống</span>
                    ...
                    <td
                      className={`p-3 truncate font-medium ${isMatched ? "text-slate-900 bg-emerald-50/10" : "text-slate-400 italic bg-rose-50/5"}`}
                      title={item.blockL2}
                    >
                      {item.blockL2 || (
                        <span className="text-slate-300 italic">Trống</span>
      ...
      {totalItems > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-1 font-mono">
          ...
          <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-500 font-bold uppercase">
            ...
            <span>
              trên <b>{totalItems}</b> hồ sơ (bản ghi {startIndex + 1} -{" "}
